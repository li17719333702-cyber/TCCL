...
def load_seu_signal(file_path: str) -> np.ndarray:
    """
    从SEU格式的.mat文件中加载驱动端(DE)的振动信号
...
class SEUDataset(Dataset):
    ...
    def __init__(self, 
        ...
        if mode == 'full':
            print(f"[SEU - 全数据集模式] 目录: {self.root_dir}")
            ...
            print(f"[SEU - 训练集模式] 目录: {self.root_dir}")
            self.dataset_mean, self.dataset_std = self._compute_dataset_stats()
            ...
            print(f"[SEU - 测试集模式] 目录: {self.root_dir}")
    ...
    def _compute_dataset_stats(self) -> tuple:
        ...
        for file_path in tqdm(mat_files, desc="收集数据", leave=False):
            signal = load_seu_signal(str(file_path))
            if signal.size > 0:
...
if __name__ == '__main__':
    DATA_ROOT = r'E:\AI\Mechanical-datasets-master\dataset'
